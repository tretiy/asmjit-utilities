...
class PerfBuilder : public asmjit::x86::Builder {
private:
	struct DebugLine {
		const char *file;
		size_t line;
...
public:
	explicit PerfBuilder(asmjit::CodeHolder *code=nullptr) noexcept : asmjit::x86::Builder(code) {}
	virtual ~PerfBuilder() {}

	// implicitly attached to latest node
	// adds file and line number as debug info to last created node (Builder::cursor())
	void attachDebugLine(const char *file_name=__builtin_FILE(), int line_number=__builtin_LINE());
